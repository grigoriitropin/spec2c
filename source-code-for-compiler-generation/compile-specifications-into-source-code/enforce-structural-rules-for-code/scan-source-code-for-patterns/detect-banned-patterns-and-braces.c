...
static int confirm_single_character_valid_as_identifier(int c) { return (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') || (c >= '0' && c <= '9') || c == '_'; }
...
int check_for_banned_keyword_pattern(const char *line) {
    for (int i = 0; i < banned_patterns_count; i++) {
        ...
        while ((p = strstr(p, pat))) {
            ...
            if (p > line && confirm_single_character_valid_as_identifier(*(p-1))) { p += plen; continue; }
            /* Check right boundary: char after match must not be ident */
            if (confirm_single_character_valid_as_identifier(p[plen])) { p += plen; continue; }
            return 1;
        }
    }
    return 0;
