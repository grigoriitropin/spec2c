...
static void extract_config_keys_from_json(cJSON *ckeys, spec_t *s) {
    if (!ckeys || !cJSON_IsArray(ckeys)) return;
    s->nconfig_keys = cJSON_GetArraySize(ckeys);
    if (s->nconfig_keys <= 0) return;
    size_t total = 0;
...
void parse_legacy_object_format_json(cJSON *root, spec_t *s) {
    ...
    if (cmds && cJSON_IsObject(cmds) && cJSON_GetArraySize(cmds) > 0)
        s->has_commands = 1;
    extract_config_keys_from_json(cJSON_GetObjectItemCaseSensitive(root, "config_keys"), s);
...
#define ADD(k, fmt, ...) do { \
    ...
    if (s->has_db) {
        ...
        ADD("db_init", "    vehir_db *db = vehir_db_open();\n    check_database_connection_is_alive(\"%s\", db);\n", s->name);
