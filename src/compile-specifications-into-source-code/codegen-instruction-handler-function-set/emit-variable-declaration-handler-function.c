...
static void emit_conditional_branch_code_primitive(cJSON *inst, FILE *out, int indent, const char *return_type) {
    ...
    if (!strcmp(ct, "numeric_compare")) {
        ...
        const char *lv  = "", *rv  = "";
        {   cJSON *l = cJSON_GetObjectItemCaseSensitive(inst, "lhs");
            cJSON *r = cJSON_GetObjectItemCaseSensitive(inst, "rhs");
            if (l && cJSON_IsObject(l)) lv = extract_json_field_string_value(l, "value");
            if (r && cJSON_IsObject(r)) rv = extract_json_field_string_value(r, "value");
        }
    ...
    if (!strcmp(op, "key_exists"))
        ...
        fprintf(out, "if (compare_slice_data_against_bytes((const uint8_t*)(%s), %s, \"%s\") == 0) {\n", tgt, current_slice_length_variable, cv ? cv : "");
    else
