...
void write_component_header_with_prototypes(const ipm_spec_t *spec, const char *hdr_path) {
    ...
    char guard[256];
    translate_hyphens_in_name(guard, modname);
    strcat(guard, "_H");
...
static void translate_hyphens_in_name(char *dst, const char *src) {
    int i = 0;
    while (src[i]) {
        if (src[i] == '-')
            dst[i] = '_';
...
void handle_ipm_spec_emit_code(const ipm_spec_t *spec, const char *out_path, int is_library) {
    ...
    if (modname[0] && out_path) {
        char hdr_path[4096];
        char cmod[256];
        translate_hyphens_in_name(cmod, modname);
    ...
    if (modname[0]) {
        char cname[256];
        translate_hyphens_in_name(cname, modname);
