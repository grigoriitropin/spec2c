            ...
            if (fn[0]) {
                if (rv[0]) {
                    ...
                    if (rt) {
                        ...
                        else if (!strcmp(fn, "process_ipm_spec_emit_code")) rc = "int";
                    }
...
void compile_every_function_into_code(const ipm_spec_t *spec, FILE *out, int is_library) {
    ...
    while (fn) {
        const char *name = fn->string;
        const char *desc = extract_json_field_string_value(fn, "description");
        cJSON *params = cJSON_GetObjectItemCaseSensitive(fn, "parameter_definitions");
        ...
        if (body && cJSON_IsArray(body)) {
            ...
            fprintf(out, ") {\n");
            generate_code_from_ast_instructions(body, out, 1, ret_c);
            fprintf(out, "}\n\n");
        }
        fn = fn->next;
    }
