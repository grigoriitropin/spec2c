...
void match_identifier_against_ast_grep(const pattern_def_t *pat, const char *identifier,
    ...
    for (int i = 0; i < nmatches; i++) {
        ...
        if (should_report) {
            append_deviation_to_report_list(dl, "forbidden-pattern", identifier, pat->severity,
    ...
    while (*p) {
        ...
        for (int i = 0; i < pat->nforbidden_strings; i++) {
            ...
            if (found && found < line_start + line_len) {
                ...
                src_copy[copy_len] = '\0';
                append_deviation_to_report_list(dl, "forbidden-string", pat->id, pat->severity,
...
void scan_for_scaffold_marker_deviations(const char *file_content, const char *spec_name,
    ...
    if (!has_vehir_lib)
        append_deviation_to_report_list(dl, "scaffold-marker", "missing-include", SEV_ERROR, 0, 0, "",
               "Missing #include \"vehir_lib.h\" — scaffold not present",
               "Add #include \"vehir_lib.h\"", -1);

    if (!has_usage)
        append_deviation_to_report_list(dl, "scaffold-marker", "missing-usage", SEV_ERROR, 0, 0, "",
               "Missing _Noreturn usage() function",
               "Add usage() function generated by spec2c", -1);

    if (!has_extern)
        append_deviation_to_report_list(dl, "scaffold-marker", "missing-extern", SEV_INFO, 0, 0, "",
               "No extern core function declaration found",
               "Add extern int <core_fn>(...) declaration", -1);

    if (!has_main)
        append_deviation_to_report_list(dl, "scaffold-marker", "missing-main", SEV_ERROR, 0, 0, "",
               "Missing int main(int argc, char *argv[])",
               "Add main() scaffold from spec2c", -1);

    if (!has_help)
        append_deviation_to_report_list(dl, "scaffold-marker", "missing-help", SEV_WARNING, 0, 0, "",
               "No --help handling found",
               "Add --help/-h check calling usage()", -1);

    if (has_system)
        append_deviation_to_report_list(dl, "scaffold-marker", "scaffold-system", SEV_ERROR, 0, 0, "",
               "system() call in scaffold — must use vl_safe_exec()",
               "Replace system() with vl_safe_exec()", -1);

    if (has_popen)
        append_deviation_to_report_list(dl, "scaffold-marker", "scaffold-popen", SEV_ERROR, 0, 0, "",
               "popen() call in scaffold — must use vl_safe_exec()",
               "Replace popen() with vl_safe_exec()", -1);

    if (!has_vl_die)
        append_deviation_to_report_list(dl, "scaffold-marker", "missing-vl-die", SEV_WARNING, 0, 0, "",
